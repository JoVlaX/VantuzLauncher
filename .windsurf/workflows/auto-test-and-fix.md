...
**Cascade ОБЯЗАН автоматически выполнять этот протокол при работе с логикой VantuzLauncher.**

**Условия обязательного запуска:**
- Любые изменения в `*.cs` файлах → **ЗАПУСТИТЬ автотест**
- Любые изменения в `*.csproj` → **ЗАПУСТИТЬ автотест**
- Рефакторинг, добавление фич, багфиксы → **ЗАПУСТИТЬ автотест**

**Исключения (не запускать):**
- Только документация (*.md)
- Только комментарии в коде
- Конфиги CI/CD без изменения логики

**Последовательность действий:**
1. Предоставить изменения пользователю
2. **АВТОМАТИЧЕСКИ** запустить `dotnet build`
3. **АВТОМАТИЧЕСКИ** запустить `test-and-run.ps1`
4. Если ошибка → **АВТОМАТИЧЕСКИ** попытаться исправить
5. Повторять до успеха или достижения лимита
6. Включить результат теста в отчёт пользователю
