...
public class MainWindow : Window, ICredentialProvider
{
    ...
    private void BuildUI()
    {
        ...
        leftStack.Children.Add(new TextBlock { Text = Properties.Resources.HeaderLogin, FontWeight = FontWeight.Bold, Foreground = Brushes.White, Margin = new Thickness(0, 0, 0, 15), FontSize = 12 });
        ...
        RememberMeBox = new CheckBox { Content = Properties.Resources.CheckRememberMe, Foreground = new SolidColorBrush(Color.FromRgb(0xAA, 0xAA, 0xAA)), Margin = new Thickness(0, 0, 0, 25), FontSize = 11 };
        ...
        PlayButton = new Button { Content = Properties.Resources.ButtonPlay, Height = 45, Background = new SolidColorBrush(Color.FromRgb(0x4C, 0xAF, 0x50)), Foreground = Brushes.White, FontWeight = FontWeight.Bold, FontSize = 14, BorderThickness = new Thickness(0) };
        AutomationProperties.SetAutomationId(PlayButton, "PlayButton");
        PlayButton.Click += PlayButton_Click;
        leftStack.Children.Add(PlayButton);

        CancelButton = new Button { Content = Properties.Resources.ButtonCancel, Height = 35, Background = Brushes.Transparent, Foreground = new SolidColorBrush(Color.FromRgb(0xAA, 0xAA, 0xAA)), FontSize = 12, BorderThickness = new Thickness(1), BorderBrush = new SolidColorBrush(Color.FromRgb(0x55, 0x55, 0x55)), Margin = new Thickness(0, 10, 0, 0), IsVisible = false };
        CancelButton.Click += CancelButton_Click;
        leftStack.Children.Add(CancelButton);

        ProgressPanel = new StackPanel { IsVisible = false, Margin = new Thickness(0, 20, 0, 0) };
        ProgressPanel.Children.Add(new TextBlock { Text = Properties.Resources.HeaderStatus, FontWeight = FontWeight.Bold, Foreground = Brushes.White, Margin = new Thickness(0, 0, 0, 10), FontSize = 12 });
        ...
        StatusText = new TextBlock { Text = Properties.Resources.StatusPreparing, TextWrapping = TextWrapping.Wrap, Foreground = new SolidColorBrush(Color.FromRgb(0xDD, 0xDD, 0xDD)), FontSize = 11 };
...
/// F_doc: {UpdateStatus returns incorrect result or throws unexpectedly} E_doc: Unit test or static analysis verifies UpdateStatus behavior
    ...
    private void PlayButton_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
        {
            ...
            AppendToLog(Properties.Resources.LogEnterCredentials);
            StatusText.Text = Properties.Resources.StatusEnterCredentials;
            return;
        }
    ...
    private void InitializeRamSlider()
    {
        ...
        RamText.Text = string.Format(Properties.Resources.RamAllocatedFormat, (int)RamSlider.Value);
    }
        ...
        try
        {
            if (File.Exists(_configPath))
            {
                ...
                if (config != null)
                {
                    ...
                    RamText.Text = string.Format(Properties.Resources.RamAllocatedFormat, ramValue);
                }
            }
        }
