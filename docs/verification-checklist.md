...
> 10. **Installer completion ≠ all artifacts present.** `ForgeInstaller.Install` returned success, created the JSON, and downloaded `fmlloader`, but did NOT download `bootstraplauncher`, `securejarhandler`, or other libraries referenced in the JSON. The vanilla path correctly called `launcher.InstallAsync` to resolve libraries; the Forge path omitted this step. The agent assumed "installer finished = ready to launch." Closed by adding `launcher.InstallAsync(installedName)` after `ForgeInstaller.Install` to download all remaining libraries before post-install verification.
> 11. **Development without documentation compliance is a recidivism.** In the same session where fixes were applied, the agent violated INVARIANT_THEORY §1.2 (no test for new path), §4.1a (Phase 18 claims without F_doc/E_doc), and COMPOSITUM_SPEC §0.3 (plan without deadlines/Meta-Compliance). The agent delivered runtime code and claimed "program works" while the code was non-compliant with the project's own architectural constitution. Closed by adding `InstallVersionAsync_ForgePath_CallsLibraryResolver` test, adding F_doc/E_doc table to Phase 18, and creating `compliance-audit-2026-06-07-forge-library.md`.
